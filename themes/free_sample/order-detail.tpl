...
{/if}
<ul class="bordercolor address item" {if $order->isVirtual()}style="display:none;"{/if}>
...
<ul class="bordercolor address alternate_item {if $order->isVirtual()}full_width{/if}">
...
<div id="order-detail-content" class="table_block">
	<table class="std">
		...
		<tfoot>
			...
			<tr class="item">
				<td colspan="{if $return_allowed}6{else}5{/if}">
					{l s='Total vouchers:'} <span class="price price-discount">{displayWtPriceWithCurrency price=$order->total_discounts currency=$currency}</span>
				</td>
			</tr>
			{/if}
			...
			<tr class="item">
				<td colspan="{if $return_allowed}6{else}5{/if}">
					{l s='Total gift-wrapping:'} <span class="price price-wrapping">{displayWtPriceWithCurrency price=$order->total_wrapping currency=$currency}</span>
				</td>
			</tr>
			{/if}
			<tr class="item">
				<td colspan="{if $return_allowed}6{else}5{/if}">
					{l s='Total shipping'} {if $use_tax}{l s='(tax incl.)'}{/if}: <span class="price price-shipping">{displayWtPriceWithCurrency price=$order->total_shipping currency=$currency}</span>
				</td>
			</tr>
			<tr class="item">
				<td colspan="{if $return_allowed}6{else}5{/if}">
			...
			{if !isset($product.deleted)}
				...
				{if isset($customizedDatas.$productId.$productAttributeId)}
					<tr class="item">
						...
						<td><label for="cb_{$product.id_order_detail|intval}">{if $product.product_reference}{$product.product_reference|escape:'htmlall':'UTF-8'}{else}&mdash;{/if}</label></td>
						<td class="bold">
						...
						<td>
							<label for="cb_{$product.id_order_detail|intval}"><span class="price">{if $group_use_tax}{convertPriceWithCurrency price=$product.product_price_wt currency=$currency convert=0}{else}{convertPriceWithCurrency price=$product.product_price currency=$currency convert=0}{/if}</span></label>
						</td>
						<td class="last_item">
							<label for="cb_{$product.id_order_detail|intval}"><span class="price">{if isset($customizedDatas.$productId.$productAttributeId)}{if $group_use_tax}{convertPriceWithCurrency price=$product.total_customization_wt currency=$currency convert=0}{else}{convertPriceWithCurrency price=$product.total_customization currency=$currency convert=0}{/if}{else}{if $group_use_tax}{convertPriceWithCurrency price=$product.total_wt currency=$currency convert=0}{else}{convertPriceWithCurrency price=$product.total_price currency=$currency convert=0}{/if}{/if}</span></label>
						</td>
					</tr>
				...
				{if $product.product_quantity > $product.customizationQuantityTotal}
					<tr class="item">
						...
						<td>
							<label for="cb_{$product.id_order_detail|intval}"><span class="price">{if $group_use_tax}{convertPriceWithCurrency price=$product.product_price_wt currency=$currency convert=0}{else}{convertPriceWithCurrency price=$product.product_price currency=$currency convert=0}{/if}</label>
						</td>
						<td class="last_item">
							<label for="cb_{$product.id_order_detail|intval}"><span class="price">{if $group_use_tax}{convertPriceWithCurrency price=$product.total_wt currency=$currency convert=0}{else}{convertPriceWithCurrency price=$product.total_price currency=$currency convert=0}{/if}</span></label>
						</td>
					</tr>
				{/if}
			{/if}
		...
		{foreach from=$discounts item=discount}
			<tr class="item">
				...
				<td>&nbsp;</td>
				<td class="last_item"><span class="price">{if $discount.value != 0.00}{l s='-'}{/if}{convertPriceWithCurrency price=$discount.value currency=$currency convert=0}</span></td>
				{if $return_allowed}
				<td>&nbsp;</td>
				{/if}
			</tr>
...
{else}
<p><img src="{$img_dir}icon/infos.png" alt="" class="icon" />&nbsp;{l s='You can\'t make a merchandise return with a guest account'}</p>
{/if}
{if !isset($smarty.get.ajax)}
	</div>
</div>
