	...
	{foreach from=$suppliers item=supplier name=suppliers}
		<li class="bordercolor"> 
			<div class="logo bordercolor">
				{if $supplier.nb_products > 0}<a href="{$link->getsupplierLink($supplier.id_supplier, $supplier.link_rewrite)|escape:'htmlall':'UTF-8'}" title="{$supplier.name|escape:'htmlall':'UTF-8'}">{/if}
			...
			<div class="left_side">
				...
				<div>
					{if $supplier.nb_products > 0}<a href="{$link->getsupplierLink($supplier.id_supplier, $supplier.link_rewrite)|escape:'htmlall':'UTF-8'}">{/if}
					{$supplier.description|escape:'htmlall':'UTF-8'|strip_tags|truncate:100:'...'}
					{if $supplier.nb_products > 0}</a>{/if}
				</div>
			</div>
			<div class="right_side bordercolor">
				<p>
					{if $supplier.nb_products > 0}<a href="{$link->getsupplierLink($supplier.id_supplier, $supplier.link_rewrite)|escape:'htmlall':'UTF-8'}">{/if}
					<span>{$supplier.nb_products|intval}</span> {if $supplier.nb_products == 1}{l s='product'}{else}{l s='products'}{/if}
					{if $supplier.nb_products > 0}</a>{/if}
				</p>
